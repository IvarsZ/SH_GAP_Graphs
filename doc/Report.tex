...
\subsection{Parallel Vertex Coloring}
...
\subsection{Parallel Minimum Spanning Trees}
...
\subsection{Serial Minimum Spanning Tree}
...
Boruvka's algorithm finds minimum weight edge for each vertex and merges the vertices accordingly repeating the process until all vertices are merged or no more edges can be selected.
...
Kruskal's algoritm also built the minimum spanning tree one edge at a time, but instead of growing one tree it grows a forest of minimum spanning trees by connecting two trees at a time. It picks the edges in ascending order of weights and excludes those that introduce cycles. Kruskal's algorithm is best in practice for graphs with low density.
...
\subsubsection{Prim's Pseudo Code}
\begin{lstlisting}[language=Ruby]
def MST(G)
...
\subsection{Serial Single-source Shortest Paths}
...
\subsection{Parallel Minimum Spanning Tree}
