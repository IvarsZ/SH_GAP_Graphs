...
TODO EXPAND TO DESCRIBE BETTER!!!!
...
Serial experiments for MST captured the average MST weight for random graphs with varied vertex count and density. To ensure uniform and comparable results MST was run on a connected graph, since Prim's algorithm out of the box works only for connected graphs. Note the implementation tested here does not have the optimization of only having one edge per vertex in the heap, since it was not implemented yet at the time when this set of experiments was run.
...
This graph displays that in practice Prim's algorithm is linear against the number of edges and the measurements are really price again, as the error bars show.
...
TODO see all charts in R with R studio.

TODO picked 2 for mstp, square cachted up weird, square might have a better sacling factor but very hard to tell.

TODO picked square for bfsp better scaling factor.
