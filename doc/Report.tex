...
To balance the queues and reduce collisions when multiple 

But instead of each thread having its own queue

The next layer is represented by a list of lists and e

The vertices of the current layer are split into some number of disjoint partitions that are processed in parallel



\cite{Leiserson, bader2006designing, cong2008solving, zhang2006parallel}

,
 vertices of equal depth, can be processed in parallel to find the vertices of the next layer
